...
time zcat ~/data/NCBI/blast/db/FASTA/nt.gz | awk 'BEGIN{ ORS = ""; RS = ">"; FS="\n" }($1~"\\| Homo sapiens" || $1~"\\| Human") && ($1~"contig" || $1~"Contig" || $1~"clone" || $1~"Clone"){ print ">" $0 }' > $BOWTIE2_INDEX_DIR/genome.contig.clone.fa

# merge files
zcat GCF_000001405.31_GRCh38.p5_genomic.fna.gz GCF_000001405.31_GRCh38.p5_rna.fna.gz | cat - $BOWTIE2_INDEX_DIR/genome.contig.clone.fa > $BOWTIE2_INDEX_DIR/genome.fa

# build samtools index 
samtools faidx $BOWTIE2_INDEX_DIR/genome.fa
# build bowtie2 index
time bowtie2-build $BOWTIE2_INDEX_DIR/genome.fa $BOWTIE2_INDEX_BASE2
